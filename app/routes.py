...
from models.dto.product_dto import ProductDTO
from models.dto.warehouse_dto import WarehouseDTO
from models.dto.category_dto import CategoryDTO
from models.dto.user_dto import UserDTO
from models.dto.role_dto import RoleDTO
from models.dto.transaction_dto import TransactionDTO, TypeTransactionDTO
...
async def create_warehouse(warehouse: WarehouseDTO):
    """ Создание склада """
    with SessionLocal() as session:
        return service.create_warehouse(session, warehouse)
...
async def create_role(role: RoleDTO):
    """ Создание категории товара """
    with SessionLocal() as session:
        return service.add_role(session, role)
...
async def add_transaction(transaction: TransactionDTO):
    """ Создание категории товара """
    with SessionLocal() as session:
        return service.add_transaction(session, transaction)
...
@router.post('/delete_product_by_id', status_code=201)
...
@router.post('/update_product_name', status_code=201)
...
@router.post('/update_warehouse_name', status_code=201)
...
@router.post('/update_warehouse_address', status_code=201)
...
@router.post('/update_category_name', status_code=201)
async def update_category_name(category_id: CategoryDTO, new_name: str):
    """ Создание категории товара """
    with SessionLocal() as session:
        return service.update_warehouse_address(session, category_id, new_name)
...
@router.post('/update_user_password', status_code=201)
...
@router.post('/update_user_role', status_code=201)
...
@router.post('/delete_transaction_by_product_id', status_code=201)
